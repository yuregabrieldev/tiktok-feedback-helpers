-- A member may have several campaigns, but must wait 30 minutes between them.
-- Administrators publish featured campaigns and are deliberately exempt.
drop index if exists public.one_active_normal_campaign_per_creator;
...
begin
  ...
  -- Serialize a member's publication attempts so two simultaneous requests
  -- cannot bypass the time limit.
  perform pg_advisory_xact_lock(42, hashtext(v_user::text));
  if not v_is_admin and exists (
    select 1 from public.campaigns
    where creator_id = v_user
      and kind = 'normal'
      and created_at > now() - interval '30 minutes'
  ) then raise exception 'campaign_cooldown'; end if;
