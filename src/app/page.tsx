        ...
        {view === 'profile' && (
          <section className="account-card">
            ...
            {profileEditing ? <form className="profile-form" onSubmit={saveProfile}>
              ...
              <div className="screenshot-field"><label className="upload-label">Screenshot do perfil TikTok<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" required={!profileDraft.tiktok_screenshot_path} onChange={(event) => void uploadProfileImage(event, 'screenshot')} />{screenshotUrl ? <small className="saved-media"><img src={screenshotUrl} alt="Screenshot salvo" />Screenshot salvo atualmente</small> : null}</label><details className="help-popover"><summary aria-label="Como deve ser o screenshot?">?</summary><p>Mostre o seu perfil do TikTok com a foto, o nome e o @ visíveis. A captura deve comprovar que o link pertence a si.</p></details><small>O @ deve estar visível.</small></div>
            ...
            {profile.is_admin && <section className="admin-panel" aria-labelledby="admin-title">
              ...
              {adminCampaigns.length === 0 ? <p className="empty-state">Ainda não existem campanhas para administrar.</p> : adminCampaigns.map((item) => adminEditingId === item.id ? <form className="profile-form admin-edit-form" key={item.id} onSubmit={(event) => void saveAdminCampaign(event, item.id)}>
                ...
                <label>Screenshot do perfil TikTok<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={(event) => void uploadProfileImage(event, 'screenshot', item.id)} />{adminMissionMedia.screenshotUrl ? <small className="saved-media"><img src={adminMissionMedia.screenshotUrl} alt="Último screenshot da missão" />Screenshot salvo atualmente</small> : <small>Nenhum screenshot salvo</small>}<details className="help-popover"><summary aria-label="Como deve ser o screenshot?">?</summary><p>Mostre o perfil do TikTok com foto, nome e @ visíveis para comprovar que o link pertence a si.</p></details></label>
